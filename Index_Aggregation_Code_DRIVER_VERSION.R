...
Index_Rebuild_Drivers <- read_excel("DII 4.0 Evolution Rebuild 240711.xlsx", sheet = "Drivers")

###### Filter Rebuild file for only indicators with codes ####

############ THIS IS A SIMPLE WAY TO REMOVE AN INDICATOR WITHOUT MESSING WITH THE DATABASE #######
####### SIMPLY DELETE ITS CODE IN THE REBUILD FILE ############

Index_Rebuild <- Index_Rebuild %>%
  filter(!Code %in% NA) %>%
  filter(Driver != "Reference") %>%
  arrange(Driver, Component, Cluster, Code)
...
Ex_Database_RTMI_LONG <- merge(Ex_Database_RTMI_LONG, Index_Rebuild[c("Code",	"Cluster", "Component", "Driver",	"Weight")], by = c("Code"), all.x = F, all.y = F) 
...
Ex_Database_RTMI_LONG_Clusters <- Ex_Database_RTMI_LONG_Indicators %>%
  ...
  mutate(value = value * Weight) %>% # weight
  #### AGGREGATE ####
  ...
  summarise(value = mean(value)) %>% #aggregate to clusters
  # remove group restictions
  ungroup() %>%
  ### Scale ####
  group_by(Cluster) %>%
  mutate(value = scale(value)) %>% #scale clusters
...
Ex_Database_RTMI_LONG_Drivers <- Ex_Database_RTMI_LONG_Components %>%
  #### Weight ####
  mutate(value = value * CompWeight) %>%
  #### Aggregate ####
  group_by(Country, Year, Driver) %>%
  summarise(value = mean(value)) %>%
  ungroup() %>%
  #### Scale ####
  group_by(Driver) %>%
  mutate(value = scale(value)) %>%
  arrange(Country, Driver, Year) %>%
  ungroup()

Ex_Database_RTMI_LONG_Drivers <- merge(Ex_Database_RTMI_LONG_Drivers, Index_Rebuild_Drivers, by = "Driver") %>% 
  arrange(Country, Driver, Year)

Ex_Database_RTMI_LONG_Scores <- Ex_Database_RTMI_LONG_Drivers %>%
  #### Weight ####
  mutate(value = value * DriverWeight) %>%
  ##### Aggregate #####
...
#### GO ON TO CODE SHEET Index_Booklet_Builder_Code_DRIVER_VERSION to CONTINUE TUTORIAL ###
